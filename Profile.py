...
is raised when attempting to load or save Profile
...
class Post(dict):
    ...
    with individual user posts. It currently supports two features:
...
class Profile:
    ...
    provided by each new user created within your program
    for a2. Pay close attention to the properties and
    functions in this class as you will need to make use
    of each of them in your program.
    ...
    it is possible for the list to not be sorted by the Post.timestamp
    property. So take caution as to how you implement your add_post code.
    ...
    True if successful and False if an invalid index was supplied.

    To determine which post to delete you must implement
    your own search operation on the posts returned from
    the get_posts function to find the correct index.
    ...
    the current instance of Profile to the file system.
    ...
    of Profile with data stored in a DSU file.
